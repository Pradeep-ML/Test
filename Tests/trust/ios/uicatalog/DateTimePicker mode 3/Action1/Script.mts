'##########################################################################################################
'Objective: Login to UICatalog and Test DateTimePicker with mode 3
...
Set objMobiDateTimePicker = MobiDevice("UICatalog").MobiDatetimePicker("DatetimePicker_Mode4")

'Call function to navigate to UIPicker screen
blnFlag = NavigateToObjectScreenUICatalog (objMobiDateTimePicker  , 4 , "datepicker"  , "pickers")
If Not blnFlag Then
	ReportStep "SelectDatePicker" , "Screen should be displayed with UIDatePicker object on it" , "Failed to open" , "N/A"
	EndTestIteration()
End If 
...
NavigateToObjectScreenUICatalog objMobiDateTimePicker  , 4 , "datepicker"  , "pickers"
...
'Step : Execute Select  with valid  hour , minutes and seconds without  prfixing zero 
'##########################################################
intStep = intStep+1
Environment("StepName") = "Step" & intStep
Environment("Description") = "Verify select with valid hour , minutes and seconds without prefixing zero"
Environment("ExpectedResult") = "Error message should be thrown"
strValue = "9:6:20"
blnResult = VerifySelect(objMobiDateTimePicker , "stringinput" , strValue , Null)
